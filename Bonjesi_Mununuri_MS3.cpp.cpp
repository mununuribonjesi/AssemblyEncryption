...
void encrypt_chars(int len, char EKey)
{
	...
	__asm {
			...
			mov  ecx, 0														// reset ecx to 0 for the loop counter

			encrypt:                                                             
            lea eax, OChars												//get the original characters of the array  
			mov eax, [eax + ecx]										// getting the next character to be encrypted from array




			push   eax												//put the value of the original char onto the stack

			lea    eax, EKey										//get the address of where the ekey is held      
			...
			pop eax                                          // returns that character and prepares it for encryption
			xor eax, edi                                    //begins encrypting the character
			rol al, 1                                      // shift the encrypted value of the source character to the left by one 
			rol al, 1                                     //  shift the encrypted value of the source character to the left by one 
			rol al, 1                                    // shift the encrypted value of the source character to the left by one 
			xor eax, edi                                // end of encryption
		    
			lea edx, EChars                           //get the encrypted value
            mov[edx + ecx], al                       //save the encrypted  value in Echars


			                                            

			inc ecx                                    //ecx++
			cmp ecx, len                              //ecx < length
			jb encrypt                               //while ecx less than length begin encryption
	...
	for (int i = 0; i < length; i++)						   //Encrypt characters one at a time
	{
		...
		__asm {												//start of assembly code 
			push   eax
				...
				cmp al, 'A'                         // Check if the characters are not upper if not it jumps to save char
				jl savechar                        // if the characters are upper it converts them to lowercase
				cmp al, 'Z'                       // Check if the characters are not upper if not it jumps to sa
				jg savechar						 // if the characters are upper it converts them to lowercase
				or al, 20h
